...
void applicationStartup(struct AccountTicketingData* Tickets, int arrSize)
{
    ...
    while (flag == 0)
    {
        arrIndex = menuLogin(Tickets->accounts, Tickets->ACCOUNT_MAX_SIZE);
        ...
        if (Tickets->accounts[arrIndex].AccntType == 'A') //Is account type of the current account an Agent (A)
        {
            menuAgent(Tickets->accounts, Tickets->ACCOUNT_MAX_SIZE, &Tickets->accounts[arrIndex]); //if yes, call menuAgent function
        }

        else if (Tickets->accounts[arrIndex].AccntType == 'C') //Is account tpe of the current account a Customer (C)
        {
...
void menuAgent(struct AccountTicketingData* Tickets, const struct Account* agentDetails)
{
    int optionNum;
    int flag = 0;
    int arrIndex = 0;
    ...
    while (flag == 0)
    {
        ...
        if (optionNum == 1) //If user selects option 1
        {
            ...
            for (i = 0; i < Tickets->ACCOUNT_MAX_SIZE; i++)
            {
                
                if (Tickets->accounts[i].AccntNum == 0)
                {
                    printf("Empty account"); //Printf used for debugging
                    //Randomize account num before entering function

                    getAccount(&Tickets->accounts[i]); //Calling function to add a new account where i variable found empty account
                    break;

                }

                if (i == Tickets->ACCOUNT_MAX_SIZE && Tickets->accounts[i].AccntNum != 0) //If the whool accounts array was went through an not open space was found
                {
        ...
        else if (optionNum == 2) //If user selects option 2
        {
            ...
            arrIndex = findAccountIndexByAcctNum(localAccountNum, Tickets->accounts, Tickets->ACCOUNT_MAX_SIZE, 1);
            ...
            {
                updateAccount(&Tickets->accounts[arrIndex]); //Calling updateAccount function, with the arrInex of the account found
            }
        ...
        else if (optionNum == 3) //If user selects option 3
        {
            ...
            else if (localAccountNum != agentDetails->AccntNum)
            {
                ...
                arrIndex = findAccountIndexByAcctNum(localAccountNum, Tickets->accounts, Tickets->ACCOUNT_MAX_SIZE, 0); 
                displayAllAccountDetailRecords(&Tickets->accounts[arrIndex], 1); //Setting it to an size of 1 so it only prints one value 
                ...
                if (removalChar == 'Y') //If user enters Y, set accountNum to 0
                {
                    ...
                    Tickets->accounts[arrIndex].AccntNum = 0;
        ...
        else if (optionNum == 4) //If user selects option 4
        {
            ...
            displayAllAccountDetailRecords(Tickets->accounts, Tickets->ACCOUNT_MAX_SIZE);

            pauseExecution();

        }

        else if (optionNum >= 5 || optionNum <= 9) //If user selects options 5 to 9, print temporarily unavailable
        {
            printf("Feature #%d currently unavailable!", optionNum);
            pauseExecution();
        }
