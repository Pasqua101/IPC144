...
int main(void)
{
    ...
    account.AccntNum = getPositiveInteger(); //Runs accountNum through getInteger function to make sure requirements are met


    //For account type
    printf("Enter the account type (A=Agent | C=Customer): ");
    account.AccntType = getCharOption("AC");
    printf("\n");
    ...
    person.birthYear = getIntFromRange(currentYear() - 200, currentYear() - 108); //High numbers needed since currentYear(); started from 2111 and not 2021
...
void displayAccount(const struct Account account, const struct UserLogin login, const struct Person person)
{
    ...
    printf("%05d", account.AccntNum);

    if (account.AccntType == 'A')
    {
        char accountType[10] = "AGENT";
        printf(" %-9s", accountType);
    }

    else if (account.AccntType == 'C')
    {
        char accountType[10] = "CUSTOMER";
        printf(" %-9s", accountType);
    }
    
    printf(" %-15s", person.Name); //Leading space is used to space out columns, can use - to visualize it
    printf(" %5d", person.birthYear); 
    
    printf(" %11.2lf", person.Income);
    printf(" %-10s", person.Country);

    printf(" %-10s", login.UserName);
    printf(" %8s", login.passwd);
    printf("\n");
